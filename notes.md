...
This was surprisingly easy for me, although I think I large part of that is due to the same reasoning given in my Service phase: my backend is simply smaller. I only had to store/access/update user login info in my database. On another note, though, MongoDB's npm library was also fairly simple to use, so I think that had a role to play as well. The only thing that tripped me up was figuring out how to properly export my database.js methods over to index.js where I could properly call my database functions. Most of my issues simply came from forgetting to add an await statement or a "db." suffix, but I eventually got the hang of it. Maybe in the future, I could come up with another function to my website (e.g. allowing users to set a certain tower as their "favorite") that would enable me to mess around with MongoDB a bit because this phase was rather interesting to learn how to code.

## WebSocket

This was much harder than I expected. WebSocket has tripped me up in previous CS classes, so I'm not surprised that I had some difficulty here, too. I found it was the most difficult to debug WebSocket. Tracking messages being sent between client and server and whether or not messages are getting caught at all or sent at all was at the root of a lot of my problems. I also had to change some React structuring by passing down some additional props to my 25 tower pages so that I can use a single WebSocket session for all of them. Design wise, I couldn't come up with much because frankly, my startup doesn't really need WebSocket communication. So, I settled for notifying the user whenever another user navigated to a tower webpage (e.g. "User2 is upgrading Monkey Ace"). Regardless of the simplicity of this design, I also found it hard to figure out when to broadcast that kind of message, but with a little bit of help, I was able to figure it out.

Originally, I planned on using WebSocket as a way of sending error messages to the user whenever they tried to make illegal upgrades to a tower (e.g. upgrading from three paths at once). But I realized that WebSocket wasn't designed for client-side-only communication, so perhaps I'll add that error functionality to my React phase 2 code on my own time, just for the fun of it.